...
export class UserModel {
  static async create(user: Omit<User, 'id' | 'created_at' | 'updated_at'>): Promise<User> {
    ...
    const [result] = await pool.execute(
      `INSERT INTO users (staff_id, first_name, last_name, email, password, department, role)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [user.staff_id, user.first_name, user.last_name, user.email, hashedPassword, user.department, user.role]
    );
  ...
  static async update(id: number, data: Partial<User>): Promise<boolean> {
    const allowedFields = ['first_name', 'last_name', 'email', 'department', 'role'];
    ...
    const values = updates.map(field => data[field.split(' ')[0] as keyof User]);
